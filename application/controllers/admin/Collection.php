...
class Collection extends Base_Controller
{
    ...
    public function searchProductWithoutCollection()
    {
        ...
        if ($collection_id == 5) {
            $carousel_products = $this->collection_model->getActiveCarouselProduct($collection_id);
            $product_ids = Hash::combine($carousel_products, '{n}.product_id', '{n}.product_id');
            $other_products = $this->collection_model->getOtherCarouselProduct($key, $product_ids);
        } else {
            $trending_products = $this->collection_model->getProductByCollection($collection_id);
            $product_ids = Hash::combine($trending_products, '{n}.product_id', '{n}.product_id');
