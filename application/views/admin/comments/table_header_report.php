...
<tr>
    ...
    foreach ($headers as $key => $row) {
        if (is_array($row) && isset($row['sorting']) && $row['sorting']) {
            ...
            $url = base_url('comment/reports') . '?' . http_build_query($conditions);
            echo '<th th-header-url="' . $url . '" class="'. ShowHeaderFAQ($row['label'].'header-crp').' th-header ' . $class.'"><span style="margin-left: 14px;">'. $row['label'] . '</span></th>';
        } else
            echo '<th class="noSortHeader-crp'.$row['label'] .' "><span style="margin-left: 14px;">'. (is_array($row) ? $row['label'] : $row) . '</span></th>';
    }
    ?>
</tr>
</thead>