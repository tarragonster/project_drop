...
class Cipush {
	...
	public function start() {
		for ($i = 1; $i < 10; $i++) {
			...
			$this->_CI->db->where('time_queued <=', time());
	...
	public function sendIOS($device_token, $payload) {
		...
		if (!$this->fp) {
			log_message('error', "Failed to connect: $err $errstr");
			return false;
		}
...
// 		print_r($result, false);
	...
	public function addAndroid($device_token, $data_send, $time_queued) {
		$params = [
			'device_type' => 1,
	...
	public function addIOS($device_token, $data_send, $time_queued) {
		$body['aps'] = $data_send;
